...
|---|---|---|---|---|---|---|---|---|---|
| Acordo Certo | Actionpay ref. 187558 — nome oficial pendente | `/go/acordo-certo` / `acordo-certo` | `https://apretailer.com.br/click/6a3f408e2bfa813aa26ff5b5/187558/359422/subaccount` | Sim | Resultado da análise (`/`) | Pendente de confirmação | Sim; resolveu para `turbinado.upone.link` | Supabase correto; GA4 recebeu payload correto com HTTP 204, mas a confirmação visual segue pendente | Presente |
| SuperSim | SuperSim — Empréstimo Pessoal | `/go/super-sim` / `super-sim` | `https://apretailer.com.br/click/6a3f408e2bfa813b02188995/177702/359422/subaccount` | Sim | Resultado da análise (`/`) | Pendente de confirmação | Sim; destino SuperSim preservou `utm_source=actionpay` e `aff_sub` | Supabase correto; envio GA4 HTTP 204; UI do GA4 pendente | Presente |
...
| Bom Pra Crédito | Bom Pra Crédito — Actionpay ref. 185636 | `/go/bom-pra-credito` / `bom-pra-credito` | `https://apretailer.com.br/click/6a3f408d2bfa813afc65b8b7/185636/359422/subaccount` | Sim | Resultado da análise (`/`) | Pendente de confirmação | Validado em produção; resolve para `afiliado.bompracredito.com.br` com parâmetros Actionpay | Supabase correto; `partner_clicked` e `affiliate_click` entregues ao GA4 com HTTP 204 | Presente com identidade própria |
...
- O Realtime e o DebugView da propriedade permaneceram zerados durante a janela do teste. A resposta HTTP 204 confirma recebimento pelo endpoint, mas não substitui a confirmação visual de processamento na interface do GA4.
